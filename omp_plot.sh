...
f=1000
reftime=`head -6 data/serial5.tsv | tail -1 | awk '{printf $5 + $6}'`
...
gnuplot <<EOF
    set terminal png
    set output "plots/speedup_total_8000.png"
...
gnuplot <<EOF
    set terminal png
    set output "plots/speedup_compute_8000.png"
    ...
    set datafile separator " "
    plot "data/openmp0_all.tsv" u 8:8 title "Serial 5 (ideal)" with linespoints, \\
         "data/openmp0_all.tsv" u 8:($reftime/(\$5+\$6)) title "openMP 0" with linespoints, \\
         "data/openmp1_all.tsv" u 8:($reftime/(\$5+\$6)) title "openMP 1" with linespoints
EOF
